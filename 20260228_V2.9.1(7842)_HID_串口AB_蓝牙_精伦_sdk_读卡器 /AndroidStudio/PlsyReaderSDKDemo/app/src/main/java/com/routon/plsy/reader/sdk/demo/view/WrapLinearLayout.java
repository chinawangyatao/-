...
public class WrapLinearLayout extends ViewGroup {
	...
	private Type mType;private List<WarpLine>  mWarpLineGroup;
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		for (int i = 0; i < mWarpLineGroup.size(); i++) {
			...
			for (int j = 0; j < warpLine.lineView.size(); j++) {
				...
				if (isFull()) {// 需要充满当前行时
					view.layout(left, t, left + view.getMeasuredWidth() + lastWidth / warpLine.lineView.size(),
							t + view.getMeasuredHeight());
					left += view.getMeasuredWidth() + mType.horizontal_Space + lastWidth / warpLine.lineView.size();
				} else {
					switch (getGrivate()) {
					case 0:// 右对齐
						view.layout(left + lastWidth, t, left + lastWidth + view.getMeasuredWidth(),
								t + view.getMeasuredHeight());
						break;
					case 2:// 居中对齐
						view.layout(left + lastWidth / 2, t, left + lastWidth / 2 + view.getMeasuredWidth(),
								t + view.getMeasuredHeight());
						break;
					default:// 左对齐
